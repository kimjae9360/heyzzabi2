...
- **AI 에이전트 3곳** (사용자가 정의한 범위):
  ...
  3. 요구사항정의서 → 업무 자동 추출(`extract-tasks`) + AI 담당자 추천(`/api/tasks/[id]/recommend-assignees`)
...
- [ ] AI 담당자 추천 이력이 영구 저장되지 않음 — `recommend-assignees`는 매번 그 자리에서 생성되는 ephemeral 응답이라, 히스토리 "에이전트" 탭에 추천 이벤트가 안 뜸. 필요하면 추천 로그 테이블 추가해야 함(4.1 참고).
